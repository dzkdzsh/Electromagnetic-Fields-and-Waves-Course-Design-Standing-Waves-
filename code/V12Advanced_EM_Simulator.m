function Advanced_EM_Simulator()
    %% 高级电磁波与传输线仿真系统 (Advanced EM Wave Simulator) v12 Legend Fix
    %
    % ========================================================================
    ...
    %   - 增强的错误处理和数值稳定性
    % 版本 v12 修复：
    %   - [修复] 3D 视图现在显示正确的图例（合成、入射、反射）而不是 2D 的包络。
    %   - [修复] Smith 图现在不再显示任何图例。
    %   - [逻辑] 在视图切换函数中增加了显式的图例可见性管理。
    % ========================================================================
    ...
    theme.white     = [1.00 1.00 1.00]; % 新增白色定义
    ...
    hFig = figure('Name', '电磁场课程设计：多维传输线综合分析平台 v12', ...
                  'NumberTitle', 'off', ...
                  'Color', theme.bg, ...
    ...
    function refresh_view_visibility(mode)
        ...
        % [修复] 显式隐藏所有图例，防止它们残留
        if isfield(H, 'leg_2d') && isvalid(H.leg_2d), set(H.leg_2d, 'Visible', 'off'); end
        if isfield(H, 'leg_3d') && isvalid(H.leg_3d), set(H.leg_3d, 'Visible', 'off'); end
        ...
        switch mode
            case '2D'
                ...
                % [修复] 仅显示 2D 图例
                if isfield(H, 'leg_2d') && isvalid(H.leg_2d), set(H.leg_2d, 'Visible', 'on'); end
                
            case '3D'
                % 3D 模式：显示立体复平面坐标轴
                set_visible(ax_3d, 'on');
                % [修复] 仅显示 3D 图例
                if isfield(H, 'leg_3d') && isvalid(H.leg_3d), set(H.leg_3d, 'Visible', 'on'); end
                
            case 'Smith'
                % Smith 图模式：显示标准化阻抗平面
                set_visible(ax_smith, 'on');
                % [修复] Smith 界面不需要任何图例，所以保持全部关闭
        end
    ...
    function H = init_graphics_objects()
        ...
        % [修复] 保存 2D 图例句柄，并设置 AutoUpdate 为 off，防止它在 3D 图中自动添加条目
        H.leg_2d = legend(ax_2d_wave, [H.v_env_up, H.v_line, H.i_line], ...
            {'Envelope', 'Voltage (V)', 'Current (I*Z0)'}, ...
            'Location', 'northwest', 'TextColor', theme.text, 'Color', theme.panel_bg, 'EdgeColor', 'none', 'AutoUpdate', 'off');
        ...
        % [修复] 创建 3D 视图的专属图例，使用正确的标签
        H.leg_3d = legend(ax_3d, [H.h3_tot, H.h3_inc, H.h3_ref], ...
            {'Synthesis (Total)', 'Incident', 'Reflected'}, ...
            'Location', 'northwest', 'TextColor', theme.text, 'Color', theme.panel_bg, 'EdgeColor', 'none', 'AutoUpdate', 'off');
