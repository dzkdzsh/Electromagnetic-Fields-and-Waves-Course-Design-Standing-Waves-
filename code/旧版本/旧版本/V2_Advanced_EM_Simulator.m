function Advanced_EM_Simulator()
    ...
    % 5. 支持滑块拖动与数值精确输入 (双向同步)
    %
    % 使用方法：直接运行此函数，在弹出的窗口中操作滑块或输入数值即可。
    ...
    sld_RL = uicontrol(panel_ctrl, 'Style', 'slider', 'Position', [20 600 200 20], ...
                       'Min', 0, 'Max', 200, 'Value', RL_init, 'Callback', @update_text_from_slider);
    % 修改为 edit 类型，允许输入
    txt_RL = uicontrol(panel_ctrl, 'Style', 'edit', 'Position', [230 600 50 20], ...
                       'String', num2str(RL_init), 'BackgroundColor', [1 1 1], ...
                       'Callback', @(h,e) update_slider_from_edit(h, sld_RL, 0, 200));
    ...
    sld_XL = uicontrol(panel_ctrl, 'Style', 'slider', 'Position', [20 540 200 20], ...
                       'Min', -200, 'Max', 200, 'Value', XL_init, 'Callback', @update_text_from_slider);
    % 修改为 edit 类型
    txt_XL = uicontrol(panel_ctrl, 'Style', 'edit', 'Position', [230 540 50 20], ...
                       'String', num2str(XL_init), 'BackgroundColor', [1 1 1], ...
                       'Callback', @(h,e) update_slider_from_edit(h, sld_XL, -200, 200));
    ...
    sld_Z0 = uicontrol(panel_ctrl, 'Style', 'slider', 'Position', [20 480 200 20], ...
                       'Min', 10, 'Max', 100, 'Value', Z0_init, 'Callback', @update_text_from_slider);
    % 修改为 edit 类型
    txt_Z0 = uicontrol(panel_ctrl, 'Style', 'edit', 'Position', [230 480 50 20], ...
                       'String', num2str(Z0_init), 'BackgroundColor', [1 1 1], ...
                       'Callback', @(h,e) update_slider_from_edit(h, sld_Z0, 10, 100));
    ...
    sld_Alpha = uicontrol(panel_ctrl, 'Style', 'slider', 'Position', [20 420 200 20], ...
                          'Min', 0, 'Max', 0.5, 'Value', alpha_init, 'Callback', @update_text_from_slider);
    % 修改为 edit 类型
    txt_Alpha = uicontrol(panel_ctrl, 'Style', 'edit', 'Position', [230 420 50 20], ...
                          'String', num2str(alpha_init), 'BackgroundColor', [1 1 1], ...
                          'Callback', @(h,e) update_slider_from_edit(h, sld_Alpha, 0, 0.5));
    ...
    % 1. 滑块回调：更新文本框显示
    function update_text_from_slider(~, ~)
    ...
    % 2. 输入框回调：验证输入并更新滑块
    function update_slider_from_edit(hEdit, hSlider, minVal, maxVal)
        str_val = get(hEdit, 'String');
        val = str2double(str_val);
        
        % 验证输入有效性
        if isnan(val)
            % 如果非数字，恢复原值
            set(hEdit, 'String', sprintf('%.1f', get(hSlider, 'Value')));
            return;
        end
        ...
        set(hEdit, 'String', sprintf('%.1f', val)); % 格式化显示
    end
