function Advanced_EM_Simulator()
    %% 高级电磁波与传输线仿真系统 (Advanced EM Wave Simulator) v4.0 Ultimate
    % --- 美化版亮点 ---
    % 1. UI 设计：全新的“暗黑科幻”主题，深色背景 + 霓虹配色。
    % 2. 视觉增强：高对比度波形绘制，优化坐标轴与网格样式。
    % 3. 核心功能：保留了 v4.0 的所有强大功能（3D、Smith 圆图、多维联动）。
    
    % --- 颜色主题定义 (Dark Sci-Fi Theme) ---
    theme.bg        = [0.10 0.10 0.12]; % 深空灰背景
    theme.panel_bg  = [0.16 0.16 0.18]; % 面板背景
    theme.text      = [0.90 0.90 0.90]; % 主文字色
    theme.text_dim  = [0.70 0.70 0.70]; % 次级文字
    theme.cyan      = [0.00 0.85 1.00]; % 高亮青 (电压)
    theme.magenta   = [1.00 0.20 0.60]; % 高亮洋红 (电流/反射)
    theme.green     = [0.20 1.00 0.40]; % 荧光绿 (SWR/包络)
    theme.yellow    = [1.00 0.80 0.20]; % 警告黄
    theme.grid      = [0.25 0.25 0.28]; % 网格线颜色
    ...
    function on_view_change(~, event)
        ...
        axis(ax_smith, 'off'); % Smith 轴永远隐藏，只画内容
    ...
    function draw_smith_view(Gamma, vswr)
        ...
        line(ax_smith, [-1 1], [0 0], 'Color', [0.5 0.5 0.5]);
    ...
    function data = prepare_smith_chart()
        ...
        for k = 1:length(x_vals)
            x = x_vals(k);
            t_arc = linspace(pi, 1.5*pi+2*atan(1/x), 60);
            cx = 1; cy = 1/x; rad = 1/x;
