function Advanced_EM_Simulator()
    %% 高级电磁波与传输线仿真系统 (Advanced EM Wave Simulator) v8.0 Ultimate
    % --- 修复与升级日志 (v8.0) ---
    % 1. UI布局重构：全线采用 'Normalized' 单位，完美支持窗口最大化和拉伸，标题不再跑偏。
    % 2. 文本渲染：修复希腊字母 alpha 显示问题，优化下标显示 (RL, Z0)。
    % 3. 3D视觉增强：
    %    - 提高波形亮度。
    %    - 新增 'quiver3' 动态箭头，指示电压矢量的实时旋转方向。
    ...
    % --- 创建主图形窗口 ---
    hFig = figure('Name', '电磁场课程设计：多维传输线综合分析平台 v8.0', ...
                  'NumberTitle', 'off', ...
                  'Color', theme.bg, ...
    ...
    try
        ...
        % 0. 顶部标题栏 [Left Bottom Width Height]
        ...
        % 创建参数滑块 (使用 normalized 布局)
        % 注意：MATLAB UI 不支持 TeX，直接使用 Unicode 字符
        ...
        % 3. 创建坐标轴 (使用 normalized 单位)
        % 2D Axes
        ax_2d_wave = axes('Parent', hFig, 'Units', 'normalized', 'Position', [0.30 0.58 0.66 0.35], ...
        ...
        ax_2d_imp = axes('Parent', hFig, 'Units', 'normalized', 'Position', [0.30 0.08 0.66 0.38], ...
            'Color', theme.bg, 'XColor', theme.text, 'YColor', theme.text, 'Tag', '2D', 'Visible', 'on');
        grid(ax_2d_imp, 'on'); hold(ax_2d_imp, 'on');
        title(ax_2d_imp, 'Impedance Magnitude |Z(z)|', 'Color', theme.text);
        
        % 3D Axes (全高)
        ax_3d = axes('Parent', hFig, 'Units', 'normalized', 'Position', [0.30 0.08 0.66 0.85], ...
        ...
        % Smith Axes (居中)
        ax_smith = axes('Parent', hFig, 'Units', 'normalized', 'Position', [0.35 0.10 0.56 0.80], ...
    ...
    function H = init_graphics_objects()
        ...
        H.h3_inc = plot3(ax_3d, NaN, NaN, NaN, 'Color', theme.cyan, 'LineWidth', 1.5); % 移除透明度，直接更亮
