function Advanced_EM_Simulator()
    %% 高级电磁波与传输线仿真系统 (Advanced EM Wave Simulator) v9 Layout Fix
    % --- 修复日志 (v9) ---
    % 1. 布局修复：下移了绘图区域，解决坐标轴标题 (Title) 与顶部大标题栏重叠的问题。
    % 2. 视觉微调：优化了 2D 两个坐标轴之间的间距。
    % 3. 核心功能保持：动画速度优化、3D 箭头、Smith 圆图等功能全部保留。
    ...
    % --- 创建主图形窗口 ---
    hFig = figure('Name', '电磁场课程设计：多维传输线综合分析平台 v9', ...
                  'NumberTitle', 'off', ...
                  'Color', theme.bg, ...
    ...
    try
        ...
        % 3. 创建坐标轴 (修正高度和位置，防止被 Title 遮挡)
        % [Left Bottom Width Height]
        % 原高度太高导致 title 顶到了上面的标题栏，现在整体缩小并下移
